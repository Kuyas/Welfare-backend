 ...
 if (isset($_POST['mobile_number']) && isset($_POST['password'])) {   
    ...
    if(!preg_match("~^[0-9]{10}$~", $_POST['mobile_number']) || 
       ...
       //connecting to db
        ...
        $query = "SELECT * FROM BANKING WHERE USER_ID IN (SELECT USER_ID FROM USER WHERE USER_MOBILE='".$mobile_number."' AND USER_PASSWORD='".$password."')";
        ...
        if($result){
            ...
            $response["bank_name"] = $result[1];
            $response["account_number"] = $result[2];
            $response["account_holder_name"] = $result[3];
            $response["bank_branch"] = $result[4];
            $response["ifsc_code"] = $result[5];        
            echo json_encode($response);
        }else{
